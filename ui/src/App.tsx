...
interface Plan {
  ...
  yaml: string;
  unresolved: string[];
  /** Secret env var names the repo asked for. Names only — never values. */
  secrets: string[];
  types: string[];
  ha: boolean;
}
...
export default function App() {
  ...
  /*
   * Separate from `created`, and it has to be. `created` describes services made INSIDE a
   * project, so the project-change effect clears it. Creating a project changes the selection,
   * which would fire that effect and wipe its own confirmation on the way in.
   */
   ...
   *
   * Creating is separate from filling on purpose: this leaves an EMPTY project, and the normal
   * scan → preview → provision path fills it with the import file shown first. A single button
   * that made a project and provisioned into it would be infrastructure appearing with nothing
   * reviewed.
   */
  const createProject = async () => guard(async () => {
    const name = (newName ?? '').trim();
    if (name === '') return;
    setError(null);
    try {
      const r = await api<{ project: { id: string; name: string; status: string } }>('/api/project', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ name, tags: ['notch'] }),
      });
      // Prepend, then select: a project you just made should be the one you are looking at.
      setProjects((cur) => [r.project, ...(cur ?? [])]);
      setProjectId(r.project.id);
      setNewName(null);
      setNotice(`Created project “${r.project.name}”. It is empty — type a repo path and scan it to see what it needs.`);
    } catch (e) { setError((e as Error).message); }
  });
  ...
  const preview = async () => guard(async () => {
    ...
    try {
      const p = await api<Omit<Plan, 'types' | 'ha'>>('/api/provision/plan', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        // `dir` so the server can read which SECRET NAMES the repo asks for. Names only; the
        // values are generated by Zerops during the import.
        body: JSON.stringify({ projectId, types: missingTypes, ha, dir: dir.trim() }),
      });
      /*
  ...
  const provision = async () => guard(async () => {
    ...
    try {
      const r = await api<{ created: { hostname: string }[]; graph: Graph | null; note: string }>('/api/provision', {
        ...
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ projectId, types: plan.types, ha: plan.ha, dir: dir.trim() }),
      });
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    /*
     * Ghosts WRAP. A single row was fine for the two or three gaps a small repo has, but a
     * realistic app needs seven, and seven times 240px is a 1680px row that forces the fit to
     * zoom out until nothing is readable — the same "correct graph drawn as confetti" failure
     * the tier layout already avoids for real services. Four to a row keeps the block roughly
     * square whatever the count.
     */
    const PER_ROW = 4;
    const ghosts: Node[] = missing.map((m, i) => ({
      id: `ghost-${m.type}`,
      type: 'service',
      position: { x: (i % PER_ROW) * 240, y: ghostY + Math.floor(i / PER_ROW) * 200 },
      data: {
    ...
    <div className="app">
      <header>
        ...
        <div className="controls">
          ...
          <button onClick={() => setNewName(newName === null ? '' : null)} disabled={busy}
            title="Create a new, empty project on this account">
            {newName === null ? 'New project' : 'Cancel'}
          </button>
      ...
      {newName !== null && (
        <div className="banner plan">
          <div className="plan-head">
            <b>Create an empty project on this account</b>
            <span>
              <input autoFocus className="dir" placeholder="project name" value={newName}
                onChange={(e) => setNewName(e.target.value)}
                onKeyDown={(e) => { if (e.key === 'Enter') void createProject(); }} />
              <button className="primary" disabled={busy || newName.trim() === ''}
                onClick={() => void createProject()}>{busy ? 'creating…' : 'Create'}</button>
            </span>
          </div>
          <div className="warn">
      ...
      {notice !== null && <div className="banner ok">{notice}</div>}
      ...
      {plan !== null && (
        <div className="banner plan">
          ...
          {plan.secrets.length > 0 && (
            <div className="warn">
              {plan.secrets.length} secret(s) found by name in your env files —{' '}
              <code>{plan.secrets.join(', ')}</code>. Zerops generates the values during the
              import (<code>generateRandomString</code>), so no secret is copied out of your
              repo and none appears in this file.
            </div>
          )}
